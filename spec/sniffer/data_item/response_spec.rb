...
RSpec.describe Sniffer::DataItem::Response do
  subject { described_class.new(headers: { 'user-agent': 'Ruby' }, body: "OK", status: 200) }

  context "#to_h" do
    it 'empty by default' do
      expect(described_class.new.to_h).to eq(status: nil, headers: nil, body: nil)
    end

    it 'returns values' do
      expect(subject.to_h).to eq(headers: { 'user-agent': "Ruby" }, body: "OK", status: 200)
    end
  end
