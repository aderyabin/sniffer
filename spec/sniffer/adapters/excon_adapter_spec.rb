...
RSpec.describe Excon do
  ...
  # let(:get_request_dynamic_params) { Excon.get("http://localhost:4567/?lang=ruby", headers: headers, params: { author: 'matz' }) }
  let(:post_request) { Excon.post('http://localhost:4567/data?lang=ruby', body: "author=Matz") }
  let(:post_json) { Excon.post('http://localhost:4567/json', json: { 'lang' => 'Ruby', 'author' => 'Matz' }) }
